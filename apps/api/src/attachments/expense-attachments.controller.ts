...
import {
  ...
  AttachmentUrlResponseDto,
  ExpenseAttachmentResponseDto,
} from './dto/expense-attachment-response.dto.js';
...
export class ExpenseAttachmentsController {
  ...
  @ApiOperation({
    summary: 'Reserve an attachment and return a presigned upload URL',
  })
  ...
  @ApiOperation({ summary: 'List active attachments for an expense' })
  @ApiOkResponse({ type: ExpenseAttachmentResponseDto, isArray: true })
  list(
    @Param('expenseId', new ParseUUIDPipe({ version: '4' })) id: string,
    @CurrentUser() user: SafeUser,
  ) {
