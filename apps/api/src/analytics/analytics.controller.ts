...
export class AnalyticsController {
  ...
  @ApiOperation({ summary: 'Derived spending/cashflow analytics; settlements and offsets excluded' })
  @ApiOkResponse({ type: AnalyticsSummaryResponseDto })
  ledger(@Param('ledgerId', new ParseUUIDPipe({ version: '4' })) id: string, @CurrentUser() user: SafeUser, @Query() query: AnalyticsQueryDto) {
    return this.service.ledger(id, user.id, query);
  }

  @Get('plans/:planId/analytics/summary')
  @ApiOkResponse({ type: AnalyticsSummaryResponseDto })
  plan(@Param('planId', new ParseUUIDPipe({ version: '4' })) id: string, @CurrentUser() user: SafeUser, @Query() query: AnalyticsQueryDto) {
