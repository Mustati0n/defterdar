...
export class ExpenseSplitOffsetsController {
  constructor(private readonly service: ExpenseSplitOffsetsService, private readonly idempotency: IdempotencyService) {}
  ...
  create(@Param('expenseSplitId', new ParseUUIDPipe({ version: '4' })) id: string, @CurrentUser() user: SafeUser, @Body() dto: CreateExpenseSplitOffsetDto, @Headers('idempotency-key') key?: string) {
    return this.idempotency.execute(user.id, `offset.create:${id}`, key, dto, () =>
